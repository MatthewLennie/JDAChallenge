...
def test_short_datafile():
    ...
    path = os.getcwd()
    path = r'file:' + \
        pathname2url(r'{}/test/test_data/Bike-Sharing-Dataset_short_real_data.zip'.format(path))
    print(path)
...
def test_empty_datafile():
    ...
    path = os.getcwd()
    path = r'file:' + \
        pathname2url(r'{}/test/test_data/Bike-Sharing-Dataset_empty_data.zip'.format(path))
    print(path)
    with pytest.raises(Exception) as e_info:
        df = read_in_data.read_in_data(path)
    assert e_info.value.args[0] == (
        "Empty file!"), "Empty file, check that source is not empty and that wasn't full of nulls"
...
def test_has_nulls():
    ...
    path = os.getcwd()
    path = r'file:' + \
        pathname2url(r'{}/test/test_data/Bike-Sharing-Dataset_has_nulls.zip'.format(path))
    df = read_in_data.read_in_data(path)
...
def test_has_extra_columns():
    ...
    path = os.getcwd()
    path = r'file:' + \
        pathname2url(r'{}/test/test_data/Bike-Sharing-Dataset_extra_column.zip'.format(path))
    df = read_in_data.read_in_data(path)
    df_check = pd.read_pickle("./test/test_data/first_row.pkl")
...
def test_has_missing_columns():
    ...
    path = os.getcwd()
    path = r'file:' + \
        pathname2url(r'{}/test/test_data/Bike-Sharing-Dataset_missing_column.zip'.format(path))
    with pytest.raises(Exception) as e_info:
        df = read_in_data.read_in_data(path)
...
def test_no_file():
    ...
    path = os.getcwd()
    path = r'file:' + \
        pathname2url(r'{}/test/test_data/asantasanasquashedbanana.zip'.format(path))
