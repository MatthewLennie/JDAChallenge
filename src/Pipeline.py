...
class Pipeline():
    ...
    def hyper_parameter_search(self):
        ...
        """
        try:
            clf = xgb.XGBRegressor(n_jobs=self.config.n_jobs,objective='reg:squarederror')
    ...
    def assess_model(self):
        ...
        with plt.style.context('seaborn-darkgrid'):
            ...
            #Do Plotting
            plt.figure()
            fig = plt.hist(abs(y_preds-self.y_test),50,range=(0,1000),density=True, Label = "Absolute Errors from Model")
            plt.vlines(mae,0,1,linewidth=4, color='r',Label="Mean Absolute Error from Model")
            ...
            #plot deviations
            plt.hist(deviations,50,density=True,range=(0,1000),Label='Absolute Deviations from data',alpha=0.4)
            plt.vlines(mad,0,1,linewidth=4, color='k',Label ='Mean Absolute Deviations')
            plt.gca().set_ylim([0,max(fig[0])*1.1])
            plt.legend()
